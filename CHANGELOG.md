...
- Implemented GitHub Actions CI/CD workflow to build and push the example site to Docker Hub as tag/latest and optionally deploy it automatically to a VPS via SSH.
- Added environment variable configurations for `SECRET_KEY`, `DEBUG`, and `ALLOWED_HOSTS` in the example site settings.
...
- Automatic 2FA bypass for administrative tools (Web Shell and File Manager) when settings.DEBUG is True.
...
## [1.0.3] - 2026-07-24
### Fixed
- Added missing Italian translations for search state messages in `change_list_results.html` ("No results found" and empty results search query guidance).

### Changed
- Configured CI/CD to automatically install `gettext` and compile translation files (`.mo` catalogs) dynamically during the workflow build.
- Updated `pyproject.toml` configuration to ensure compiled `.mo` translation catalogs are properly bundled into releases.

## [1.0.2] - 2026-07-24
### Added
- Added `DjangoAdminJSMiddleware` to automatically capture and format AJAX/Fetch exceptions on admin pages.
- Introduced `ERROR_LEVEL` config setting to customize exception toast display: `generic`, `title` (default), and `stacktrace`.
- Implemented dynamic, smoothly-expandable error toast panels for scrollbar-free traceback reading.
...
- Removed all beta notices and warnings from documentation and README.
